...
class SelfPlay:
    ...
    def __init__(self, initial_checkpoint, Game, config, seed):
        ...
        self.device = torch.device("cuda:0" if self.config.selfplay_on_gpu and torch.cuda.is_available() else "cpu")
        print(self.device)
    ...
    def self_play(self, shared_storage, replay_buffer, writer, train_step,test_mode=False,num = 1):
        for _ in range(num):
            self.model.set_weights(shared_storage.get_info("weights"))
            # print(shared_storage.get_info("weights"))
            # print(self.training_step,self.config.training_steps)
            ...
            #         time.sleep(0.5)
        self.close_game()
        # writer.add_scalar('1.Total_reward/1.Total_reward', shared_storage.get_info('total_reward'), train_step)
        # writer.add_scalar('1.Total_reward/2.Mean_value', shared_storage.get_info('mean_value'), train_step)
        # writer.add_scalar('1.Total_reward/3.Episode_length', shared_storage.get_info('episode_length'), train_step)
        # writer.add_scalar('1.Total_reward/4.Muzero_reward', shared_storage.get_info('muzero_reward'), train_step)
        # writer.add_scalar('1.Total_reward/5.Opponent_reward', shared_storage.get_info('opponent_reward'), train_step)
    ...
    def play_game(
        ...
        with torch.no_grad():
            while (
                ...
                # Move stacked_observations to the same device as the model

                stacked_observations = torch.tensor(stacked_observations).float().to(self.device)
...
class MCTS:
    ...
    def run(
        ...
        """
        在搜索树的根部，我们使用表示函数来获取一个
        隐藏状态。
        然后，我们仅使用动作序列和网络学习到的模型运行蒙特卡洛树搜索。
        进行蒙特卡洛树搜索。
        ...
        if override_root_with:
            ...
            root = Node(0)
            # Use clone().detach() to avoid the UserWarning
            device = next(model_to_use.parameters()).device  # Get the device of the model
            observation = observation.clone().detach().float().unsqueeze(0).to(device)  # Move to the same device
